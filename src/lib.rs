...
#![doc(html_root_url = "https://docs.rs/fs-err/3.1.3")]
#![deny(missing_debug_implementations, missing_docs)]
#![cfg_attr(docsrs, feature(doc_cfg))]

// Emit a helpful compile-time warning if the debug feature is enabled on Rust < 1.79
#[cfg(all(feature = "debug", not(rustc_1_79)))]
compile_error!("The `debug` feature requires Rust 1.79 or later due to the `path_facts` dependency. Please upgrade your Rust version or disable the `debug` feature.");
